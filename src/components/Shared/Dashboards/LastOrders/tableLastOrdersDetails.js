...
export default function OrderDetailsClient({ order }) {
  ...
  const handlePrintInvoice = () => {
    ...
    if (!alreadyId) {
      ...
      localStorage.setItem("dashboardsSummary", JSON.stringify(invoices));
    }
      ...
      <div className="p-6 max-w-4xl mx-auto ">
        <div ref={contentRef}>
          <Card>
            ...
            <CardContent>
              <p>
                <b>Ürün Adı:</b> {order.name}
              </p>
              <p>
                <b>Kategori:</b> {order.category}
              </p>
              <p>
                <b>Fiyat:</b> {order.price}
              </p>
              <p>
                <b>Stok:</b> {order.stock}
              </p>
              <p>
                <b>Kritik Stok:</b> {order.criticalStock}
              </p>
              <p>
                <b>Satılan Ürün Sayısı:</b> {order.sold}
              </p>
              ...
              <p>
                <b>Tarih:</b> {formatDate(order.createdAt)}
              </p>
              <p>
                <b>Toplam:</b> {totalPrice}
              </p>
            </CardContent>
          </Card>
        </div>
        ...
        <div className="flex justify-end mt-4 gap-1">
          <Button onClick={handlePrintInvoice} className="bg-[#6C120B]">
            Faturayı Yazdır
          </Button>
          <Button variant="outline" onClick={() => downloadDashboardPDF(order)}>
            PDF İndir
          </Button>
          <Updated order={order} />
        </div>
